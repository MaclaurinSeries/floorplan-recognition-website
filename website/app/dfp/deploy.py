...
def main(config: argparse.Namespace, image: Image.Image) -> np.ndarray:
    model, img, shp = init(config, image)
    if config.loadmethod == "log":
    ...
    #     r[cw != 0] = 0
    #     return (r + cw).squeeze()
    if config.colorize and not config.postprocess:
        r_color, _ = colorize(r.squeeze(), cw.squeeze())
        return r_color
    ...
    newr_color, _ = colorize(newr.squeeze(), newcw.squeeze())
